...
describe('SendResetPasswordEmail Controller', () => {
  ...
  it('Should return bad request if email is not registered', async () => {
    const { sut, sendResetPasswordEmailStub } = makeSut()
    jest.spyOn(sendResetPasswordEmailStub, 'send').mockReturnValueOnce(new Promise((resolve, reject) => reject(new NotFoundError('email'))))
    const httpRequest = {
      body: {
  ...
  it('Should return a message on success', async () => {
    ...
    expect(httpResponse.body).toEqual({ message: 'Email sent' })
  })
})
